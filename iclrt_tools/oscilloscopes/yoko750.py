...
# Program to read and process Yokogawa DL750 ".HDR" and ".WVF" files
#
...
class Yoko750File(object):
    """
    This class implements an object for a Yokogawa DL750 file, which contains a header and traces.
    """

    def __init__(self, fileName):
        if fileName[-3:]=='HDR' or fileName[-3:]=='WVF':
            filename=filename[:-4]
        self.fileName = fileName

    ###########################################################################
    # get_header()
    # This function processes the '.HDR' file.
    # Returns a YokoHeader object.
    def get_header(self):
        """
...
#~             result = None
    ...
    ###########################################################################
    # get_trace_data()
    # This function processes the '.WVF' file.
    # Returns a YokoTrace object.
...
##                    for i in range(thousandSamples.shape[0]):
...
##                    data = data - thousandSampleOffset
                    ...
                    """
#~                     data-=np.mean(data[0:1000])
                    data-=np.mean(data[-1000:])
