...
##                                      unknown=9
            ...
            self.Data = np.fromstring(original_data_string,
                                      dtype=self.endianness + self.data_type)
            
            self.Data = self.Data * self.vertical_gain - self.vertical_offset
            
    def get_trig_time_array_data(self):
        return np.fromstring(self.trig_time_array_data,
                             dtype=self.endianness + 'f8')
        
    def get_ris_time_array_data(self):
        return np.fromstring(self.ris_time_array_data,
                             dtype=self.endianness + 'f8')
