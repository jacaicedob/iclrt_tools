...
def di_r(zp,tp,r):
    ...
    fs_delay = tp - r/c - Hstart/v[0] # Delay due to free space propagation to 
                            # observation pt.
    
    ### ONLY FOR REF FROM GND
    #~ fs_delay -= 2.0*Hr/v[0]
    ###
...
def P(zp):
    ...
    #### ONLY FOR REF GOING UP
    #~ return 1
    ####
    
    if direction == 'up':
        return np.exp(-(zp-Hstart)/lmb[j])
    else:
        return 1
...
# Geometric constants
Ht = 4.5E3          # Top of channel (m)
Hstart = 355    # Height at which desired wave starts
Hr = 355    # Height of discontinuity (only used in RS reflection problem)
v_bounds = [7.1E7, 7.1E7]           # Return stroke speed (m/s)
l_bounds = [2E3, 2E3]   # Lambda boundaries bottom to top
...
try:
    ...
    fileName = "/media/jaime/JaimeBackup/OSX2/My Papers/Current Reflections/UF 09-29 063009/Waveforms/Scope24/D0005"
    ...
    calFactor = 63000 # Specifies the calibration factor for each
                                    # trace
    df32_tStart = 9.99939084E-1 # Gives the starting time of interest for
                                        # each trace
    df32_tStop = 1.00075924 # Gives the ending time of interes for
                                    # each trace
    ...
    cft = cf.CurrentFitTail(t_array_i, i-dc.DCoffset, 25.0E-6)
    cft.fit()
    i = cft.i
    t_array_i = cft.t
...
t_array = np.arange(0,30E-6,delta_t)
...
pickle.dump(data, open('./data_%s.p' % direction, "wb"))
#~ pickle.dump(data, open('./data_Ref.p', "wb"))
#~ pickle.dump(data, open('./data_RS.p', "wb"))
