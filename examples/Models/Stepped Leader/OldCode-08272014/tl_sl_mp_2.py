...
def E_es_integrand(tp, z):
    # Returns the integrand for the electrostatic component of E
    r = R(z)
    return ((2.0-3.0*(D/r)**2)/r**3)*I_r(z,tp,r)*np.exp(-(z-Hb)/decay)
    #~ return I_r(z,tp,r)
...
def di_r(z,tp,r):
    ...
    fs_delay = tp - (r/c*1E6)   # Delay due to free space propagation to 
                        # observation pt.
    ...
    if fs_delay < 0:
        ...
        z_delay = fs_delay - ((z-Hb)/v*1E6)   # Delay due to velocity of wave. The
...
#~     result_E_es = result_E_es[0]*1E-3

#~     result_E_es = integrate.dblquad(E_es_integrand, \
#~                                     RR/c*1E6,t, # tp (tprime) limits
#~                                     lambda zp:Hb, #z limits
#~                                     lambda zp:Ht, \
#~                                     epsabs=1.49E-15, \
#~                                     epsrel=1.49E-15)
...
#~     result_E_es = result_E_es[1]*1E-3
    opts = {'epsabs':1.49E-9, 'epsrel':1.49E-9, 'limit':1000}
    result_E_es = integrate.nquad(E_es_integrand, \
                                  [[RR/c*1E6, t],[Hb, Ht]], \
                                  opts=[opts, opts])
...
#~     result_E_ind = result_E_ind[0]*1E3
...
#~     result_E_rad = result_E_rad[0]*1E9
...
if __name__ == "__main__":
    ...
    RR = 300    # meters
    theta = np.array([0]) * np.pi / 180 #np.array([90]) * np.pi / 180 # radians
    ...
    #~ i *= 1E6    # Fix units since my code uses time in seconds and the units of 
                # dI/dt in the paper are us-1
...
#~     sys.exit(1)
    ...
    for xx in xrange(DD.shape[0]):
        ...
        t_max = ((Ht-Hb)/v + np.sqrt(Ht**2+D**2)/c)*1E6 + t_end_i # Largest time to 
                                                              #compute fields
                                                              # in useconds
        t_array = np.arange(RR/c*1E6,t_max,delta_t)    # Time array for loop iteration
        
        print "Simulation start time: %s" % time.strftime("%H:%M:%S", time.localtime())
        print ""
        print "D = %0.2f meters" % (D)
        print "%0.2f < z' < %0.2f meters" % (Hb, Ht)
        print "%0.2f < t < %0.2f us" % (t_array[0], t_array[-1])
        print "%0.2f < r < %0.2f meters" % (R(Hb), R(Ht))
        print "delta_t = %0.2f ns" % (delta_t*1E3)
        #~ sys.exit(1)
...
#~         p6 = df.plotdf(fig, ax2)
    ...
    #~ 
        ...
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.plot((t_array-D/c*1E6),dE_tot*1E-3)
